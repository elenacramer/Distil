...
from lstm_utils.loss import WeightedMSE
from lstm_utils.modeling_lstm import SimpleLSTM
from lstm_utils.text_utils import normalize
...
class _LSTMBase(Trainer):
    ...
    def epoch_evaluate_func(self, model, eval_dataset, epoch):
        ...
        predictions = None
        labels = None
        for i, (text, xlm_prob, real_label) in enumerate(tqdm(data_loader, desc='Val')):
            text, xlm_prob, real_label = self.to_device(text, xlm_prob, real_label)
            output = model(text.t()).squeeze(1)
...
class LSTMDistilledWeighted(LSTMDistilled):
    ...
    def loss(self, output, xlm_prob, real_label):
        ...
        l2 = (1 - self.a) * self.criterion_mse(output, xlm_prob, real_label)
        return l1 + l2