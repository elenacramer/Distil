...
class Trainer(object):
    def __init__(self, settings, logger):
        ...
        self.tb_writer = SummaryWriter(
            log_dir=settings["log_dir"], filename_suffix=settings["tb_suffix"]
        )

    def log_epoch(self, train_loss, eval_loss, acc, epoch):
        self.logger.info("######## epoch={} ########".format(epoch))
        self.logger.info(
            "train_loss={:.4f}, val_loss={:.4f}, acc={:.4f}".format(
                train_loss, eval_loss, acc
            )
        )

        self.tb_writer.add_scalars(
            "{}/loss".format(self.settings["tb_suffix"]),
            {"train": train_loss, "test": eval_loss},
            epoch,
        )
        self.tb_writer.add_scalar(
            "{}/val_acc".format(self.settings["tb_suffix"]), acc, epoch
        )

    def log_pr(self, labels, predictions, epoch):
        self.tb_writer.add_pr_curve(
            "{}/pr".format(self.settings["tb_suffix"]),
            labels,
            predictions,
            global_step=epoch,
        )
        self.tb_writer.add_scalar(
            "{}/ap_score".format(self.settings["tb_suffix"]),
